...
//   users[0]=Ticket(0,"admin","url",buyPrice,noOfTicketAvailable,typeOfTicket);
  ...
  function addTicket(uint id,uint seatno,string calldata color,string calldata additionalfeature,uint status) external  onlyOwner{
    // uint count=ticketTypes[id].tickets.length;

    Ticket[] memory temp =ticketTypes[id].tickets;
    uint count=temp.length;

    if(status==0){
  Ticket memory ticket=Ticket(count,seatno,color,additionalfeature,Status.Available);
  ticketTypes[id].tickets.push(ticket);
    }else{
  Ticket memory ticket=Ticket(count,seatno,color,additionalfeature,Status.InUse);
  ticketTypes[id].tickets.push(ticket);
    }
